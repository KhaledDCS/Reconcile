...
export async function getTransactions() {
  ...
  if (error) throw error;
  return data.map(t => ({ id: t.id, date: t.date, vendor: t.vendor, description: t.description, amount: parseFloat(t.amount), card: t.card, userId: t.user_id, gl: t.gl, glName: t.gl_name, dept: t.dept, confidence: t.confidence, autoAssigned: t.auto_assigned, isRecurring: t.is_recurring, assigneeId: t.assignee_id, autoAssignee: t.auto_assignee, status: t.status, memo: t.memo, flagReason: t.flag_reason, reconId: t.recon_id, receipt: t.receipts?.[0] ? { name: t.receipts[0].name, size: t.receipts[0].size, url: t.receipts[0].url } : null, receiptMatch: t.receipts?.[0]?.receipt_match || null }));
}

export async function updateTransaction(id, changes) {
  const keyMap = { userId:'user_id', glName:'gl_name', autoAssigned:'auto_assigned', isRecurring:'is_recurring', assigneeId:'assignee_id', autoAssignee:'auto_assignee', flagReason:'flag_reason', reconId:'recon_id' };
  const mapped = {};
